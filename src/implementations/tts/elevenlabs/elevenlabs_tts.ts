...
export class ElevenlabsTTS extends EventEmitter<TTSEvents> implements TTS {
  ...
  constructor({ apiKey, url, voiceId, headers, queryParameters, timeout, queueSizeLimit }: ElevenlabsTTSOptions) {
    ...
    this.url =
      url ??
      `wss://api.elevenlabs.io/v1/text-to-speech/${voiceId ?? "JBFqnCBsd6RMkjVDRZzb"}/multi-stream-input?${qs.stringify(
        { ...{ model_id: "eleven_flash_v2_5", output_format: "ulaw_8000" }, ...queryParameters }
      )}`;
